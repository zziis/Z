/* Taj Al Molook V27.2 — installed APK launch + protected downloads + faster rendering */
(()=>{
  'use strict';
  ...
  function normalizePackage(v){
    v=String(v||'').trim();
    return /^[A-Za-z][A-Za-z0-9_]*(?:\.[A-Za-z][A-Za-z0-9_]*)+$/.test(v)?v:'';
  }
  const directUrl=x=>normalizeOpenUrl(x?.open_url||x?.launch_url||x?.web_url||x?.website_url||'');
  const packageName=x=>normalizePackage(x?.package_name||x?.android_package||x?.package_id||'');
  ...
  function setOpenButton(openBtn,done,pkg,hasDirect){
    if(!openBtn)return;
    const label=openBtn.querySelector('span');
    openBtn.disabled=!!current?.locked;
    if(done&&pkg){
      openBtn.hidden=false;
      if(label)label.textContent='فتح';
      openBtn.title='فتح التطبيق المثبّت على الجهاز';
      return;
    }
    if(hasDirect){
      openBtn.hidden=false;
      if(label)label.textContent='فتح مباشر';
      openBtn.title='فتح الرابط المباشر بدون تنزيل';
      return;
    }
    // APK-only item: show Open only after download so the user can launch the installed package.
    openBtn.hidden=!done;
    if(label)label.textContent='فتح';
    openBtn.title=done&&!pkg?'أضف Package Name من حساب المطور حتى يعمل زر فتح':'فتح التطبيق';
  }
  function setButtons(){
    if(!current)return;
    const done=isDownloaded(current.id),locked=!!current.locked,n=ensureLockedNote(),openBtn=$('r2OpenBtn'),downloadBtn=$('r2DownloadBtn'),pkg=packageName(current),hasDirect=!!directUrl(current);
    if(n){n.hidden=!locked;n.textContent=current.lock_message||'🔒 أوقف المطور تنزيل هذا العنصر مؤقتًا.'}
    setOpenButton(openBtn,done,pkg,hasDirect);
    if(downloadBtn){downloadBtn.hidden=locked||done;downloadBtn.disabled=false}
    if($('r2InstallBtn'))$('r2InstallBtn').hidden=!done||locked;
    if($('r2InstallNote'))$('r2InstallNote').hidden=!done||locked;
    if(done&&!locked){$('r2ProgressWrap').hidden=false;$('r2ProgressText').textContent='100%';$('r2ProgressBar').style.width='100%'}else if(!locked){$('r2ProgressWrap').hidden=true;$('r2ProgressText').textContent='0%';$('r2ProgressBar').style.width='0%'}else{$('r2ProgressWrap').hidden=true}
  }
  ...
  function launchInstalledPackage(pkg){
    if(!pkg)return false;
    try{
      // Preferred path when the Android wrapper exposes a JS bridge.
      if(window.AndroidApp&&typeof window.AndroidApp.launchPackage==='function'){
        const result=window.AndroidApp.launchPackage(pkg);
        if(result===false)toast('التطبيق غير مثبت على الجهاز. ثبّت ملف APK أولًا.');
        return true;
      }
      if(window.Android&&typeof window.Android.launchPackage==='function'){
        const result=window.Android.launchPackage(pkg);
        if(result===false)toast('التطبيق غير مثبت على الجهاز. ثبّت ملف APK أولًا.');
        return true;
      }
    }catch(e){console.warn('native launch bridge',e)}
    try{
      // Chrome/compatible Android WebViews can hand this URI to the installed package.
      const intent=`intent:#Intent;package=${encodeURIComponent(pkg)};action=android.intent.action.MAIN;category=android.intent.category.LAUNCHER;end`;
      close();
      window.location.href=intent;
      return true;
    }catch(e){console.warn('intent launch',e);return false}
  }
  function openDirect(){
    if(!current||current.locked)return;
    const done=isDownloaded(current.id),pkg=packageName(current),url=directUrl(current);
    if(done&&pkg){
      if(!launchInstalledPackage(pkg))toast('تعذر فتح التطبيق. تأكد أنه مثبت وأن Package Name صحيح.');
      return;
    }
    if(done&&!pkg&&!url){
      toast('أضف Package Name لهذا الـAPK من حساب المطور حتى يعمل زر فتح بعد التثبيت.');
      return;
    }
    if(!url){
      toast('هذا APK فقط. نزّله وثبّته أولًا، ثم سيظهر زر فتح عند إضافة Package Name.');
      return;
    }
    try{
      close();
      const a=document.createElement('a');a.href=url;a.target='_self';a.rel='noopener noreferrer';a.style.display='none';document.body.appendChild(a);a.click();a.remove();
      setTimeout(()=>{try{if(document.visibilityState==='visible')window.location.assign(url)}catch(_){}},80);
    }catch(e){try{window.location.href=url}catch(_){toast('تعذر فتح الرابط المباشر. تأكد من الرابط في لوحة المطور.')}}
  }

...
-- Keep the developer account as ID 1.
update public.profiles set role='developer' where user_number=1;

-- Secure code creation. Assumes subscription_codes exists from V5/V6.
create or replace function public.admin_create_code(p_type text, p_duration_minutes integer)
returns text language plpgsql security definer set search_path=public as $$
declare v_code text;
begin
 if not public.is_developer() then raise exception 'developer only'; end if;
 if p_type not in ('trial','basic') or p_duration_minutes < 1 then raise exception 'invalid code settings'; end if;
 v_code := 'TAJ-' || upper(substr(md5(random()::text || clock_timestamp()::text),1,4)) || '-' || upper(substr(md5(random()::text),1,4));
 insert into public.subscription_codes(code,code_type,duration_minutes) values(v_code,p_type,p_duration_minutes);
 return v_code;
end $$;

...
async function openRoom(room){
  ...
  $("roomWelcomeNotice").textContent=room.welcome_text?`👋 ${room.welcome_text}`:'';
...
async function loadRoomMessages(){
  ...
  const {data,error}=await sb.from('messages').select('id,room_id,user_id,content,media_type,media_path,media_duration,created_at,profiles(display_name,username)').eq('room_id',currentRoom.id).order('created_at',{ascending:true}).limit(250);
  if(error){$("messages").innerHTML="<div class='empty'>تعذر تحميل الرسائل.</div>";return;}
  const ids=(data||[]).filter(m=>m.user_id===currentUser.id).map(m=>m.id);
  let counts={};
  if(ids.length){
  ...
  $("messages").innerHTML='';(data||[]).forEach(m=>appendMessage({...m,read_count:counts[m.id]||0},false));scrollBottom();
}
...
async function openDm(user){
  ...
  await markDmDelivered(user.id);await markDmRead(user.id);await loadDmMessages();await loadDmList();await updateUnreadBadge();closeMobileSidebar();
  ...
  if(messageChannel)await sb.removeChannel(messageChannel);
  messageChannel=sb.channel('dm-'+[currentUser.id,user.id].sort().join('-')).on('postgres_changes',{event:'*',schema:'public',table:'private_messages'},async p=>{
    ...
    if(p.eventType==='INSERT'&&m.receiver_id===currentUser.id){await markDmDelivered(user.id);await markDmRead(user.id);playChatSound('receive');}
    await loadDmMessages();await loadDmList();await updateUnreadBadge();
  }).subscribe();
}
...
async function markDmDelivered(otherId){
  await sb.from('private_messages').update({delivered_at:new Date().toISOString()}).eq('sender_id',otherId).eq('receiver_id',currentUser.id).is('delivered_at',null);
}
async function markDmRead(otherId){
  const now=new Date().toISOString();
  await sb.from('private_messages').update({delivered_at:now,read_at:now}).eq('sender_id',otherId).eq('receiver_id',currentUser.id).is('read_at',null);
}
...
function enableComposer(on){
  $("messageInput").disabled=!on;$("sendBtn").disabled=!on;
  $("imageBtn").disabled=!on;$("voiceBtn").disabled=!on;
  if(!on && mediaRecorder?.state==="recording") cancelVoiceRecording();
}
...
$("voiceBtn").onclick=async()=>{
  ...
  try{
    ...
    recordedChunks=[];recordedBlob=null;recordedDuration=0;
    let options={};
    ...
    show($("recordBar"),true);$("voiceBtn").classList.add('recording');
    recordTimer=setInterval(()=>{
      recordedDuration=Math.floor((Date.now()-recordStartedAt)/1000);
      const mm=String(Math.floor(recordedDuration/60)).padStart(2,'0'),ss=String(recordedDuration%60).padStart(2,'0');
      $("recordTime").textContent=`${mm}:${ss}`;
      if(recordedDuration>=120)stopVoiceRecording();
    },250);
  }catch(err){alert('يجب السماح للتطبيق باستخدام الميكروفون.');}
};
function stopVoiceRecording(){
  if(mediaRecorder?.state==='recording')mediaRecorder.stop();
  clearInterval(recordTimer);recordTimer=null;$("voiceBtn").classList.remove('recording');
}
function cancelVoiceRecording(){
  if(mediaRecorder?.state==='recording')mediaRecorder.stop();
  clearInterval(recordTimer);recordTimer=null;recordedChunks=[];recordedBlob=null;recordedDuration=0;
  $("voiceBtn").classList.remove('recording');show($("recordBar"),false);$("recordTime").textContent='00:00';
}
$("cancelRecordBtn").onclick=cancelVoiceRecording;
$("sendRecordBtn").onclick=async()=>{
  if(mediaRecorder?.state==='recording'){stopVoiceRecording();await new Promise(r=>setTimeout(r,180));}
  if(!recordedBlob?.size)return cancelVoiceRecording();
  const blob=recordedBlob,duration=recordedDuration;
  show($("recordBar"),false);$("recordTime").textContent='00:00';recordedBlob=null;recordedChunks=[];
  const ext=blob.type.includes('mp4')?'m4a':'webm';
  const file=new File([blob],`voice-${Date.now()}.${ext}`,{type:blob.type});
  await sendMediaFile(file,'audio',duration);
};
...
function goBackToList(){
  ...
  show($('roomWelcomeNotice'),false);show($('roomOnlineCount'),false);show($('deleteRoomBtn'),false);
